...
Barahona demonstrated that finding the ground state 
of a 3D spin-glass is an NP-hard problem
...
**A point to clarify: Do their approaches
also demonstrate the difficulty of the pure Ising model,
which is a lot simpler than the general spin glass problem?**

The problem I would like to address is the misleading
statement by SCIAM that the Ising model cannot
be solved analytically.
And also the interesting question on whether there
is a solution for the infinite limit,
or the continuous limit of the 3D Ising model.
...
There was some confusion on my part whether the
3D Ising model were undecidable or NP-complete,
and my thoughts now is that it can be both,
although no "real problem" as of now has been demonstrated
to be undecidable, in the Gödel sense, only some
very philosophical and foundational issues in Mathematics.
Something on this line would be the conjecture that
a polynomial solution to the Ising model cannot be found,
if this were undecidable, we would have a real problem
undecidable, the same with the Goldbach conjecture.
